...
pub async fn evaluate_agents(
    ...
    info!("🎯 Evaluating agents for item: {}", req.item_description);
    ...
    info!("✅ Evaluated {} agents, winner: {}", evaluations.len(), evaluations[0].agent_name);
...
pub async fn get_network_graph(
    ...
    info!("🗺️ Fetching network graph for user: {}", claims.email);
    ...
    let agent_rows = sqlx::query(
        "SELECT 
            ...
            a.transaction_count,
            (SELECT t2.id FROM agent_team_members tm2 
             JOIN agent_teams t2 ON t2.id = tm2.team_id 
             WHERE tm2.agent_id = a.id 
             ORDER BY tm2.joined_at DESC LIMIT 1) as team_id,
            (SELECT t2.team_color FROM agent_team_members tm2 
             JOIN agent_teams t2 ON t2.id = tm2.team_id 
             WHERE tm2.agent_id = a.id 
             ORDER BY tm2.joined_at DESC LIMIT 1) as team_color,
            COALESCE(
                (SELECT array_agg(t2.id ORDER BY tm2.joined_at)
                 FROM agent_team_members tm2
                 JOIN agent_teams t2 ON t2.id = tm2.team_id
                 WHERE tm2.agent_id = a.id),
                ARRAY[]::uuid[]
            ) as team_ids,
            COALESCE(
                (SELECT array_agg(t2.team_name ORDER BY tm2.joined_at)
                 FROM agent_team_members tm2
                 JOIN agent_teams t2 ON t2.id = tm2.team_id
                 WHERE tm2.agent_id = a.id),
                ARRAY[]::text[]
            ...
            COALESCE(
                (SELECT array_agg(t2.team_color ORDER BY tm2.joined_at)
                 FROM agent_team_members tm2
                 JOIN agent_teams t2 ON t2.id = tm2.team_id
                 WHERE tm2.agent_id = a.id),
                ARRAY[]::text[]
    ...
    for row in agent_rows {
        let team_ids: Vec<Uuid> = row.try_get("team_ids").unwrap_or_default();
    ...
    info!("✅ Network graph generated: {} nodes, {} edges", nodes.len(), edges.len());
